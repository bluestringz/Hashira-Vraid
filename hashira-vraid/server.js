...
app.post('/api/auth', (req, res) => {
  ...
  if (mode === 'register') {
    ...
    if (U(k)) return bad('May gumagamit na ng username na yan');
    db.users[k] = { name, pass: hash(pw), role: 'default' };
  } else if (!U(k) || !same(pw, U(k).pass)) return bad('Mali ang username o password');
  if (db.banned[k]) return bad('Naka-ban ang account na ito');
